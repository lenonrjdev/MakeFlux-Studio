        ...
        <header className="flex items-start justify-between gap-4 border-b border-[#e1e6e5] bg-white px-5 py-4">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[8px] text-[#909798]">{rotuloEtapaAtual(tarefa.etapaAtual)}</span>
            </div>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          <section className="painel-superficie rounded-md p-4">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="rounded-md border border-[#e5e9e9] bg-[#fafbfb] p-3"><span className="block text-[7px] uppercase tracking-[0.055em] text-[#9aa1a2]">SaÃ­da</span><strong className="mt-1 block text-[8.5px] text-[#4b5253]">{tarefa.formato} Â· {tarefa.qualidade.split(" Â· ")[0]}</strong></div>
              <div className="rounded-md border border-[#e5e9e9] bg-[#fafbfb] p-3"><span className="block text-[7px] uppercase tracking-[0.055em] text-[#9aa1a2]">VersÃµes</span><strong className="mt-1 block text-[8.5px] text-[#4b5253]">{tarefa.quantidadeVersoes} saÃ­da(s)</strong></div>
          ...
          {tarefa.erro && (
            <section className="mt-4 rounded-md border border-[#ebcccc] bg-[#fff6f5] p-4">
              ...
              <div className="mt-3 space-y-2 rounded-md border border-[#efdddd] bg-white/70 p-3 text-[8px] leading-4 text-[#7a6260]">
                <p><strong>Causa provÃ¡vel:</strong> {tarefa.erro.causaProvavel}</p>
                <p><strong>AÃ§Ã£o sugerida:</strong> {tarefa.erro.acaoSugerida}</p>
          ...
          <section className="mt-4 painel-superficie rounded-md p-4">
            ...
            <p className="mt-1 text-[8px] text-[#92999a]">A retomada poderÃ¡ continuar a partir da etapa que falhou.</p>
            <div className="mt-3"><ProgressoEtapas etapas={tarefa.etapas} /></div>
          </section>

          <section className="mt-4 painel-superficie rounded-md p-4">
            <div className="flex items-center justify-between gap-4">
              <div><h3 className="flex items-center gap-2 text-[10px] font-semibold text-[#303637]"><FolderOpen className="size-3.5 text-[#74817e]" /> {conteudoProducao.arquivosTitulo}</h3><p className="mt-1 text-[8px] text-[#92999a]">Resultados vinculados a esta renderizaÃ§Ã£o.</p></div>
              <span className="rounded-full bg-[#edf4f2] px-2 py-1 text-[8px] font-medium text-[#287565]">{tarefa.arquivos.length}</span>
            </div>
            <div className="mt-3 space-y-2">
              {tarefa.arquivos.length === 0 ? <div className="rounded-md border border-dashed border-[#dfe4e4] px-3 py-6 text-center text-[8px] text-[#949b9c]">Os arquivos aparecerÃ£o apÃ³s a finalizaÃ§Ã£o.</div> : tarefa.arquivos.map((arquivo) => { const Icone = iconesArquivo[arquivo.tipo]; return <div key={arquivo.id} className="flex items-center gap-3 rounded-md border border-[#e4e8e8] bg-[#fafbfb] p-3"><span className="grid size-8 shrink-0 place-items-center rounded-md bg-white text-[#6f7d79] ring-1 ring-[#e1e6e5]"><Icone className="size-3.5" /></span><div className="min-w-0 flex-1"><strong className="block truncate text-[8.5px] font-medium text-[#4b5253]">{arquivo.nome}</strong><span className="mt-1 block truncate text-[7px] text-[#969d9e]">{arquivo.tamanho} Â· {arquivo.caminho}</span></div></div>; })}
            </div>
          </section>

          <section className="mt-4 painel-superficie rounded-md p-4">
            <div className="flex items-center justify-between gap-4"><div><h3 className="flex items-center gap-2 text-[10px] font-semibold text-[#303637]"><TerminalSquare className="size-3.5 text-[#74817e]" /> {conteudoProducao.logsTitulo}</h3><p className="mt-1 text-[8px] text-[#92999a]">Mensagens simplificadas e histÃ³rico tÃ©cnico da tarefa.</p></div>{!["concluida", "erro", "cancelada"].includes(tarefa.status) && <button type="button" onClick={aoSimularErro} className="foco-acessivel inline-flex h-7 items-center gap-1.5 rounded-md px-2 text-[7.5px] text-[#91654d] hover:bg-[#faf2ec]"><TriangleAlert className="size-3" /> Simular falha</button>}</div>
          ...
          <div className="mt-4 flex items-center justify-between gap-3 rounded-md border border-[#e7e1d9] bg-[#fbf8f4] p-3 text-[8px] leading-4 text-[#826d58]"><span>Identificador: {tarefa.id.slice(0, 18)}â€¦</span>{podeExcluir && <button type="button" onClick={aoExcluir} className="foco-acessivel inline-flex items-center gap-1.5 rounded px-2 py-1 text-[#a04d4d] hover:bg-white"><Trash2 className="size-3" /> Excluir</button>}</div>
        </div>
      </aside>
    </div>
  );
