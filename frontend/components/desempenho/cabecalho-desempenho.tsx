...
export function CabecalhoDesempenho() {
  return (
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-[8px] font-semibold uppercase tracking-[0.1em] text-[#1d8b74]">
            <Gauge className="size-3" /> Sistema
          </div>
          <h1 className="mt-2 text-[22px] font-semibold tracking-[-0.035em] text-[#252a2b]">
            {conteudoDesempenho.titulo}
          </h1>
          <p className="mt-1.5 max-w-[760px] text-[10px] leading-5 text-[#788181]">
            {conteudoDesempenho.descricao}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <SeloStatus texto="Versão 1.1.0" tom="verde" />
          <span className="flex h-9 items-center gap-2 rounded-md border border-[#dfe7e5] bg-[#f7faf9] px-3 text-[9px] text-[#527068]">
            <Database className="size-3.5 text-[#1f9078]" />
            Schema SQLite v2
          </span>
        </div>
      </div>
