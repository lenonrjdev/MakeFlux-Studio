...
export function CentralBiblioteca() {
  ...
  function sincronizarFontes() {
    ...
    notificar(
      total > 0
        ? `${total} novos recursos sincronizados da ProduÃ§Ã£o e do LaboratÃ³rio de IA.`
        : "A biblioteca jÃ¡ estÃ¡ sincronizada com os mÃ³dulos locais.",
    );
  }
  ...
  function criarNovaColecao(nome: string) {
    ...
    notificar(`ColeÃ§Ã£o â€œ${colecao.nome}â€ criada.`);
  }

  function removerColecaoComConfirmacao(id: string) {
    const colecao = colecoes.find((item) => item.id === id);
    if (!colecao) return;
    if (!window.confirm(`Remover a coleÃ§Ã£o â€œ${colecao.nome}â€? Os recursos nÃ£o serÃ£o excluÃ­dos.`)) return;
    removerColecao(id);
    setColecaoSelecionada("todos");
    notificar("ColeÃ§Ã£o removida. Os recursos continuam disponÃ­veis.");
  }

  function excluirComConfirmacao(id: string) {
    const recurso = recursos.find((item) => item.id === id);
    if (!recurso) return;
    if (!window.confirm(`Excluir â€œ${recurso.nome}â€ da biblioteca? O arquivo fÃ­sico nÃ£o serÃ¡ apagado nesta fase.`)) return;
    excluirRecurso(id);
    if (recursoSelecionadoId === id) setRecursoSelecionadoId(null);
    notificar("ReferÃªncia removida da biblioteca.");
  }
  ...
  function usarNoProjeto() {
    ...
    if (!transferencia) {
      notificar("Este tipo de recurso nÃ£o pode ser aplicado diretamente ao estÃºdio.", "aviso");
      return;
    }
  ...
  async function copiarCaminho() {
    ...
    try {
      ...
      notificar("Caminho copiado para a Ã¡rea de transferÃªncia.");
    } catch {
      notificar("NÃ£o foi possÃ­vel copiar o caminho automaticamente.", "aviso");
    }
  }
          ...
          <main className="min-w-0 space-y-3">
            ...
            {recursosFiltrados.length > 0 ? (
              <GradeRecursosBiblioteca
                ...
                aoDuplicar={(id) => {
                  duplicarRecurso(id);
                  notificar("ReferÃªncia duplicada na biblioteca.");
                }}
