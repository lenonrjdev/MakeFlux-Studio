    ...
    <div className="space-y-4">
      <CartaoConfiguracao titulo="Versões instaladas" descricao="Aplicativo desktop e motor principal de geração.">
        ...
        <div className="flex items-center justify-between border-t border-[#edf0f0] pt-4">
          ...
          <Botao variante="primario" disabled={verificando} onClick={() => void aoVerificar().then((resultado) => aoNotificar(resultado.mensagem))}><RefreshCw className={`size-3.5 ${verificando ? "animate-spin" : ""}`} /> {verificando ? "Verificando..." : "Verificar agora"}</Botao>
        </div>
      </CartaoConfiguracao>
      <CartaoConfiguracao titulo="Política de atualização" descricao="Configure como novas versões serão tratadas quando o atualizador nativo estiver conectado.">
